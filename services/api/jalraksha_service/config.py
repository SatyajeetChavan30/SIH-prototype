...
def _env(key: str, default: str) -> str:
    """
    Read an environment variable, honouring the pre-rename name.

    The project was called FloodView until 2026-09-11, and a shell profile, a
    scheduled task or a Docker env file set up before then still exports
    FLOODVIEW_*. Silently ignoring those would, for instance, drop the Earth
    Engine project and turn every live GEE layer into a refusal. So a
    JALRAKSHA_* key falls back to its FLOODVIEW_* twin, and says so once.
    """
    if key in os.environ:
        return os.environ[key]
    if key.startswith("JALRAKSHA_"):
        legacy = "FLOODVIEW_" + key[len("JALRAKSHA_"):]
        if legacy in os.environ:
            if legacy not in _LEGACY_ENV_WARNED:
                _LEGACY_ENV_WARNED.add(legacy)
                print(f"[config] using legacy {legacy} for {key} "
