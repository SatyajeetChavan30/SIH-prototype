...
def _adopt_legacy_sqlite(database_url: str) -> None:
    """
    Rename the pre-rename database file (floodview.db) to the configured one.

    The project was called FloodView until 2026-09-11 and its sqlite file was
    data/floodview.db. Without this, the first connection after the rename would
    create an EMPTY data/jalraksha.db beside the real one, and every stored run
    would vanish from the picker while still sitting on disk.

    Done here, at connect time, rather than at config import: DATABASE_URL is
    RELATIVE and script_runs.registered_run chdirs to the repo root after
    importing config, so an import-time check could look in the wrong
    directory, miss the legacy file, and let the empty database be created.

    RENAMED, never copied: two databases that diverge silently is exactly the
    failure script_runs.py exists to prevent.
    """
    from pathlib import Path
    ...
    legacy = target.with_name("floodview.db")
    if target.name == legacy.name or target.exists() or not legacy.exists():
        return
    legacy.rename(target)
    print(f"[db] renamed legacy database {legacy} -> {target} "
          "(project renamed FloodView -> JalRaksha)", flush=True)


def _connect() -> Any:
    url = settings.DATABASE_URL
    _adopt_legacy_sqlite(url)
