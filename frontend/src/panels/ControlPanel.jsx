...
/**
 * Colour the run-status pill by state.
 *
 * Status is a free-form string assembled at several call sites ("idle",
 * "queued 1a2b3c4d", "Solving member 12/30 — 41%", "failed: …", "loaded …"),
 * so it is classified here rather than tracked as a separate enum — one place
 * to change if a new phrase is added, and no risk of the words and the colour
  ...
  return (
    <div className="sidebar">
      <div className="sidebar-title">
        <h3>Scenario</h3>
        <span className="sub">{TARGET_RESOLUTION_M} m grid</span>
      </div>
      ...
      {selectedDam?.note && <div className="note note--warn">{selectedDam.note}</div>}
      ...
      {!isBlockage && <>
        <div className="field">
          <div className="field-head">
            ...
            <span className="field-value">{heightM} m</span>
          </div>
        ...
        <div className="field">
          <div className="field-head">
            <span>Storage</span>
            <span className="field-value">{storage} MCM</span>
          </div>
      ...
      <div className="section">
        ...
        <div className="field">
          <div className="field-head">
            ...
            <span className="field-value">{ensemble}</span>
          </div>
        ...
        <div className={`status ${statusTone(status)}`}>{status}</div>
      </div>

      {currentRunId && (
        <div className="section">
          <div className="section-label">Desktop 3D</div>
          {/*
          ...
          */}
          ...
          {pvStatus && <div className="hint">{pvStatus}</div>}
        </div>
      )}
      ...
      <div className="section">
        ...
        <div style={{ marginTop: 6, display: "flex", gap: 6 }}>
          ...
          <button className="btn-sm" style={{ flex: "0 0 auto" }}
                  onClick={() => loadExisting()}>Load</button>
        </div>
      </div>
    ...
    <fieldset>
      ...
      {source === "detect" && (
        <div style={{ marginTop: 8 }}>
          <button className="btn-sm btn-block" onClick={detect} disabled={detecting}>
      ...
      <div className="field">
        <div className="field-head">
          <span>Crest height above the valley floor</span>
          <span className="field-value">{crestHeightM} m</span>
        </div>
      ...
      <div className="field">
        <div className="field-head">
          <span>Crest width across the valley</span>
          <span className="field-value">{widthM} m</span>
        </div>
        <input type="range" min="100" max="4000" step="50" value={widthM}
               onChange={(e) => setWidthM(+e.target.value)} />
        {/* A sub-grid barrier is a modelling error, not a note in passing, so
            it changes class rather than only colour. */}
        <div className={subGrid ? "note note--error" : "hint"}>
...
function DetectionResult({ detection, onUseManual }) {
  ...
  return (
    <div className={`note ${refused ? "note--warn" : "note--ok"}`}>
      <strong>
      ...
      {refused && (
        <button className="btn-sm" onClick={onUseManual} style={{ marginTop: 6 }}>
...
function GeeBadge({ gee }) {
  ...
  return (
    <div className={`note ${ok ? "note--ok" : "note--warn"}`} style={{ marginTop: 12 }}>
...
function DamClassWarning({ hazard }) {
  ...
  return (
    // Kept at a heavier weight than the other notes: it is the caveat that
    // most changes how a peak-outflow figure should be read.
    <div className="note note--warn"
         style={{ marginTop: 12, fontSize: 11, borderLeftWidth: 5 }}>
  ...
  return (
    <div className="section">
      <div className="section-label">
        Gauges {hasRun ? "— arrival time" : "— reference"}
      </div>
      {!hasRun && (
...
export function PopulationAtRisk({ data }) {
  ...
  return (
    <div className="section">
      <div className="section-label">Population at risk</div>
      <div className="metric-value">{n(par.total_par)}</div>
      ...
      <table className="data-table" style={{ marginTop: 8 }}>
        <tbody>
          <tr>
            <td>&lt; 15 min warning</td>
            <td className="num"><strong>{n(par.par_high_urgency_under_15min)}</strong></td>
          </tr>
          <tr>
            <td>15–60 min</td>
            <td className="num"><strong>{n(par.par_medium_urgency_15_60min)}</strong></td>
          </tr>
          <tr>
            <td>&gt; 60 min</td>
            <td className="num"><strong>{n(par.par_low_urgency_over_60min)}</strong></td>
          </tr>
        </tbody>
      </table>
      <div className="hint">
  ...
  return (
    <div className="section">
      ...
      <div style={{ display: "flex", gap: 6 }}>
        <button style={{ flex: 1 }} onClick={() => setPlaying((p) => !p)}>
          {playing ? "❚❚ Pause" : "▶ Play"}
        </button>
        <button aria-label="Previous frame" onClick={prev}>◀</button>
        <button aria-label="Next frame" onClick={next}>▶</button>
      </div>
      <input type="range" min="0" max={keyframes.length - 1} value={index}
             onChange={(e) => seekTo(+e.target.value)} style={{ marginTop: 8 }} />
