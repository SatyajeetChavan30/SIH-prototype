...
class TestSphBackendSelection:
    """
    SPH runs on the GPU only through PySPH's OpenCL backend, whose kernels are
    generated by compyle. compyle 0.9.1 is broken on Python 3.14 in two ways and
    jalraksha/sph/compyle_compat.py repairs both, so the GPU is reachable here —
    but the probe must keep refusing whatever the repair does NOT cover, and
    "auto" must then fall back to the CPU and say why, because an auto choice
    that picked the GPU and crashed would lose the whole run.
    """

    def test_cpu_request_is_honoured(self):
        choice = resolve_sph_backend("cpu")
        assert choice["sph_backend"] == "cpu" and choice["argv"] == []

    def test_auto_never_picks_a_gpu_that_cannot_be_programmed(self, monkeypatch):
    ...
    def test_explicit_opencl_request_raises_when_it_cannot_run(self, monkeypatch):
        monkeypatch.setattr(
            pysph_runner, "_compyle_gpu_incompatibility", lambda: "compyle uses ast.Str")
        with pytest.raises(SPHUnavailableError, match="ast.Str"):
            resolve_sph_backend("opencl")

    def test_gpu_path_keeps_float64(self, monkeypatch):
        """The OpenCL path must pass --use-double; PySPH's GPU default is float32."""
    ...
    def test_unknown_backend_is_rejected(self):
        with pytest.raises(ValueError):
            resolve_sph_backend("cuda-please")

    def test_one_dashboard_control_governs_both_engines(self):
        """
    ...
    def test_an_unknown_solver_backend_falls_back_to_auto(self):
        """
        Unknown here must NOT raise. This maps a value that already passed the
        API's own validation; a new SWE backend name appearing later should cost
        the near-field run its GPU, not the whole run.
        """
        assert sph_backend_for_solver("tpu") == "auto"

    def test_a_gpu_request_degrades_to_cpu_with_a_recorded_reason(self, monkeypatch):
        """
        The deliberate asymmetry with the SWE solver, which RAISES instead: SPH
        is a supplementary product, so a GPU that cannot run costs the run its
        speed and not its result -- and the reason reaches the payload, so no
        label can claim a GPU that did not run.
        """
        monkeypatch.delenv(SPH_BACKEND_ENV, raising=False)
        monkeypatch.setattr(
    ...
    def test_still_water_gate_on_the_gpu_when_it_can_run(self):
        ...
        result = run_still_water_validation(
            depth_m=3.0, spacing_m=0.5, duration_s=0.6, tank_cells=8, backend="opencl")
        assert result["sph_backend"] == "opencl"
...
GPU_SKIP = _gpu_skip_reason()
requires_gpu_sph = pytest.mark.skipif(
    GPU_SKIP is not None, reason=GPU_SKIP or "")


@pytest.fixture(scope="module")
def gpu_run():
    if GPU_SKIP is not None:
        pytest.skip(GPU_SKIP)
    return _small_run(backend="opencl")


@requires_gpu_sph
class TestGpuNearField:
    """
    The SAME physical gates as TestPySPHNearField, run on the OpenCL backend.

    Each backend has to pass on its own physics. There is deliberately no
    trajectory comparison against the CPU: the Cython NNPS and the GPU octree
    NNPS sum neighbour contributions in different orders, so forces differ at
    round-off from the first step and a collapsing water column amplifies that.
    `tests/test_solver_cuda.py` takes the same stance for the SWE solver, and
    for the same reason.
    """

    def test_the_payload_names_the_gpu(self, gpu_run):
        assert gpu_run["sph_backend"] == "opencl"
        assert "OpenCL" in gpu_run["sph_backend_label"]
        assert "float64" in gpu_run["sph_backend_label"]

    def test_all_positions_finite(self, gpu_run):
        for axis in ("x", "y", "z", "u", "v", "w"):
            assert np.all(np.isfinite(gpu_run[axis])), f"{axis} has non-finite values"

    def test_respects_the_energy_bound(self, gpu_run):
        assert gpu_run["max_speed_m_s"] <= gpu_run["energy_bound_m_s"], (
            f"max speed {gpu_run['max_speed_m_s']:.1f} m/s exceeds the "
            f"available-head bound {gpu_run['energy_bound_m_s']:.1f} m/s"
        )

    def test_most_particles_stay_over_the_terrain(self, gpu_run):
        escaped_fraction = gpu_run["n_escaped"] / gpu_run["n_fluid"]
        assert escaped_fraction < 0.25, (
            f"{escaped_fraction:.0%} of particles left the domain"
        )

    def test_surge_front_advances_downstream(self, gpu_run):
        front = gpu_run["front_position_m"]
        assert len(front) > 1
        assert front[-1] >= front[0], "the surge front must not retreat upstream"

    def test_reports_no_gauge_arrivals(self, gpu_run):
        assert gpu_run["reaches_downstream_gauges"] is False

    def test_both_backends_agree_on_the_integral_picture(self, gpu_run, baseline_run):
        """
        A LOOSE cross-backend check, on integral quantities only.

        Equality is not available and not wanted (see the class docstring). What
        must hold is that the two backends describe the same event: the same
        particle count from the same deterministic generator, a front that
        advances in both, and both maxima inside the same energy bound. A GPU
        that silently solved nothing would pass none of these.
        """
        assert gpu_run["n_fluid"] == baseline_run["n_fluid"]
        assert gpu_run["energy_bound_m_s"] == pytest.approx(
            baseline_run["energy_bound_m_s"], rel=1e-9)
        for run in (gpu_run, baseline_run):
            assert run["max_speed_m_s"] <= run["energy_bound_m_s"]
            assert run["front_position_m"][-1] >= run["front_position_m"][0]
