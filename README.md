...
| `JALRAKSHA_SPH_ENGINE` | Near-field SPH engine — `auto`, `dualsphysics` or `pysph`. | `auto`: DualSPHysics when an install is found, otherwise PySPH, with the choice and reason on the result (`sph_engine`, `sph_engine_reason`). An explicitly named engine that cannot run **raises**. |
| `JALRAKSHA_DUALSPHYSICS_DIR` | DualSPHysics v5.4 install directory (the unzipped release; `bin/windows/` must hold `DualSPHysics5.4_win64.exe` and `DualSPHysics5.4CPU_win64.exe`). Never bundled — LGPL-2.1, run as an external program. | `<repo>/DualSPHysics_v5.4`. A set-but-wrong path is reported, never searched around. |
| `JALRAKSHA_SPH_BACKEND` | Near-field SPH hardware — `auto`, `gpu` (`cuda`/`opencl` accepted) or `cpu`. Overrides a run's `auto`. | `auto`: GPU when it can run (DualSPHysics's CUDA build, probed by running it; or PySPH's OpenCL path), otherwise the CPU with the reason. `gpu` is strict start to finish: no device **raises**, and a GPU run that fails partway is an error, never a CPU rerun. A dashboard "GPU only" (`cuda`) run is strict for SPH too. |
