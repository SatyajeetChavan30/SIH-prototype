# FloodView Coefficient Verification Log
...
```python
# floodview/terrain/breach.py (Phase 3)
...
floodview run --dam tehri --allow-unvetted

# Production mode (requires all coefficients verified):
floodview run --dam tehri  # Fails if unvetted items used
```
...
Implementation: floodview/terrain/breach.py, `froehlich_regression()`
