...
| 39 | Evacuation-directive thresholds (`DIRECTIVE_LEAD_TIME_S`, `DIRECTIVE_HAZARD_FLOOR`, `EVACUATE_HAZARD_LEVEL`) | Phase 6 | ❌ TODO | `impact/evacuation.py`. Three chosen values turn a gauge's FD2320 class (depth-only) and its median arrival time into EVACUATE / PREPARE / MONITOR / NO_ARRIVAL: `DIRECTIVE_LEAD_TIME_S = 7200` s (a wet gauge reached sooner is escalated to EVACUATE), `DIRECTIVE_HAZARD_FLOOR = LOW` (the weakest class that prompts any action) and `EVACUATE_HAZARD_LEVEL = SIGNIFICANT` (evacuate whatever the lead time). **None is from published guidance.** The two-hour figure is a working estimate of the time an organised evacuation of a riverside settlement needs; mapping FD2320's "significant — danger for most" description onto an evacuation threshold is a judgement. **Gated by label, not by refusal:** `DIRECTIVE_THRESHOLDS_VERIFIED = False` is echoed as `thresholds_unvetted` in every directive payload with the values themselves, and the Gauges and Impact tabs render a Caveat beside any directive carrying it (DECISIONS.md §17). `tests/test_evacuation.py` asserts ORDERING (more hazard or less lead never weakens a directive), not these numbers, so revising them needs no test rewrite. Transcribe a lead-time basis (CWC Emergency Action Plan guidance for dams, or NDMA flood guidelines) and a hazard-to-action mapping, then flip the flag. Separate from `WARNING_LEAD_TIME_S` (PAR buckets, `tasks.py`), which answers a different question and is not reused here. | Defra/EA FD2320 (2006) class descriptions; no published directive mapping cited | — | — |
